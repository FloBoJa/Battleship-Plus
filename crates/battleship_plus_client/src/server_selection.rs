...
impl Plugin for ServerSelectionPlugin {
    fn build(&self, app: &mut App) {
        ...
        app.init_resource::<UiState>()
            ...
            .add_startup_system(initialize_ui_state)
...
fn initialize_ui_state(mut ui_state: ResMut<UiState>, user_name: Res<lobby::UserName>) {
    ui_state.user_name = user_name.to_string();
}
...
fn draw_selection_screen(
    ...
    egui::CentralPanel::default().show(egui_context.ctx_mut(), |ui| {
        ui.vertical_centered(|ui| {
            ...
            ui.horizontal(|ui| {
                ui.label("User name:           ");
                let name_text_edit = ui.text_edit_singleline(&mut ui_state.user_name);
                if name_text_edit.lost_focus() {
                    commands.insert_resource(lobby::UserName(ui_state.user_name.clone()));
                }
            });
        });
    });
...
fn draw_server_list_row(
    row: &mut egui_extras::TableRow,
    server: Entity,
    server_information: &networking::ServerInformation,
    ...
    row.col(|ui| {
        ...
        match server_information.security {
            Confirmed(AuthoritySigned) => ui.colored_label(Color32::GREEN, "\u{2713} (CA-signed)"),
            Confirmed(SelfSigned) => ui.colored_label(Color32::YELLOW, "\u{2713} (self-signed)"),
            Confirmed(NoVerification) => ui.colored_label(Color32::RED, "\u{2713} (unsigned)"),
